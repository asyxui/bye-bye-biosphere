...
func create_slot(slot_id: String) -> bool:
	...
	# A directory alone is not a usable slot. Create the first valid JSON file
	# before reporting success so an interrupted first save can be recovered.
...
func save_game(slot_id: String) -> void:
	...
	# Save to JSON file using a temporary file and backup of the last canonical
	# version. The voxel database is intentionally independent of this write.
	var slot_dir = get_slot_directory(slot_id)
...
func load_game_data(slot_id: String) -> Variant:
	...
	# A valid temporary or backup file is enough to recover the slot. Only the
	# primary malformed file is retained; stale recovery artifacts are ignored.
	...
	# No usable JSON remains. Reset non-voxel state and repair the canonical file.
	if FileAccess.file_exists(canonical):
		_preserve_corrupt_file(canonical)
