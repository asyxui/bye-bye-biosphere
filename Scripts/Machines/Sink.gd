...
func _physics_process(delta: float) -> void:
	...
	while _consume_accumulator >= 0.25:
		...
		if extracted != null and extracted.quantity > 0:
			...
			GameplayEventBus.publish(GameplayEventBus.ITEM_DELIVERED, extracted.item_id, extracted.quantity)
			_update_label()
...
func _on_intake_body_entered(body: Node3D) -> void:
	...
	if delivered_item != null:
		GameplayEventBus.publish(GameplayEventBus.ITEM_DELIVERED, delivered_item.id, 1)
	_update_label()
	drop.queue_free()
