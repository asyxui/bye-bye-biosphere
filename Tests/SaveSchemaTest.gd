...
func _run() -> void:
	check(SaveManager.CURRENT_SAVE_VERSION == 2)
	check(SaveManager.validate_save_data({"game_metadata": {"version": 2}}))
	check(not SaveManager.validate_save_data({}))
	check(SaveManager.get_last_load_error() == SaveManager.INCOMPATIBLE_SAVE_MESSAGE)
	check(not SaveManager.validate_save_data({"game_metadata": {"version": 1}}))
	check(SaveManager.get_last_load_error() == SaveManager.INCOMPATIBLE_SAVE_MESSAGE)
